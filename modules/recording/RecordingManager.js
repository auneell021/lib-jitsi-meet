...
class RecordingManager {
     ...
     *
     * @param {string} jibriQueueJID - The JID of the server side component that manages the jibri queue.
     ...
     */
    _obtainToken(jibriQueueJID, session) {
        if (typeof jibriQueueJID === 'undefined') {
     ...
     * @param {string} [options.jibriQueueJID] - The JID of the server side component that manages the jibri queue.
    ...
    startRecording(options) {
        ...
        return this._obtainToken(jibriQueueJID, session).then(({ isCanceledByUser, token }) => {
            if (isCanceledByUser) {
                return Promise.resolve();
            }
